...
TheÂ `Export-Clixml`Â cmdlet creates a Common Language Infrastructure (CLI) XML-based representation of an object or objects and stores it in a file. You can then use theÂ `Import-Clixml`Â cmdlet to recreate the saved object based on the contents of that file.
